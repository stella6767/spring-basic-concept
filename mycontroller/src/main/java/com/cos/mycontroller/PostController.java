...
public class PostController {
	...
	//데이터 전송이 목적이면 Post를 사용
	
	@PostMapping("/test/post")
	public String testPost2(String username, String password) { //x-www-form-urlencoded(기본전략)
		return "username: " + username + " password: " + password;
	}
	...
	public User testPostJson(@RequestBody User user) { 
		...
		return user;//MessageConvertor가 낚아챔 -> json으로 변환해서 리턴(RestController 일때만)
	}
